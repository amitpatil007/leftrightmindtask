...
@objc open class ACFloatingTextfield: UITextField {
    ...
    //left contain
    @IBInspectable open var isContainLeftIcon :Bool = false
    ...
    override open func textRect(forBounds bounds: CGRect) -> CGRect {
        return CGRect(x:50, y:4, width:bounds.size.width, height:bounds.size.height);
...
fileprivate extension ACFloatingTextfield {
    ...
    func addFloatingLabel(){
        ...
        labelPlaceholder?.font = UIFont.init(name: (self.font?.fontName ?? "roboto.regular")!, size: 12)
        ...
        let leadingConstraint = NSLayoutConstraint.init(item: labelPlaceholder!, attribute: .leading, relatedBy: .equal, toItem: self, attribute: .leading, multiplier: 1, constant: 50)
    ...
    func floatPlaceHolder(selected:Bool) -> Void {
        ...
        labelPlaceholder?.font = UIFont(name: (self.font?.fontName)!, size: 12) // 16
